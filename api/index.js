...
export default async function handler(req, res) {
    ...
    if (pathname === '/api/admin/create-license') {
        ...
        if (!bot_token || !chat_id) return res.status(400).json({ error: 'bot_token and chat_id are required' });
    ...
    if (pathname === '/api/send') {
        ...
        if (!license_key || !text) return res.status(400).json({ error: 'license_key and text are required' });
        const record = await kv.get(`license:${license_key}`);
        if (!record) return res.status(403).json({ ok: false, error: 'Invalid license key' });
        if (!record.active) return res.status(403).json({ ok: false, error: 'License revoked' });
        const { bot_token, chat_id } = record;
        await kv.set(`license:${license_key}`, { ...record, last_used: new Date().toISOString() });
        try {
            const tgRes = await fetch(`https://api.telegram.org/bot${bot_token}/sendMessage`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ chat_id, text, parse_mode: parse_mode || 'HTML', disable_web_page_preview: true }),
            });
